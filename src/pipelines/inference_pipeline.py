...
def extract_dataframe_from_bigquery(project_id, dataset_extract_id, table_extract_id) -> pd.DataFrame:  
   ...
   try:
       ...
       query = f"SELECT * FROM `{project_id}.{dataset_extract_id}.{table_extract_id}` WHERE fecha_ingreso >= DATE_SUB(CURRENT_DATE(), INTERVAL 30 DAY)" 
...
def generate_batch_predictions(df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        df_original = df
        input_df = df_original.drop(['clienteidentifier', 'fecha_ingreso'], axis=1)
        ...
        df_final = pd.concat([df_original[['clienteidentifier', 'fecha_ingreso']], predictions_df], axis=1)
        mapping = {'seniorcitizen': {'Y': '1', 'N':'0'},'prediction': {1: 'YES', 0:'NO'}}
        df_final.replace(mapping, inplace=True)
